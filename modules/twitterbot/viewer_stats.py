...
class vstats():
    ...
    def __init__(self, token, stime, workdir, channel, dbid=None, test=None):
        ...
        if os.path.exists(os.path.join(workdir, 'analytics/vstats.tmp')):
    ...
    def collect_data(self):
        ...
        reversed_change_title = self.change_title[::-1]
        for title in reversed_change_title:
            if title != None and title[1] != get_data('game_id'):
        ...
        try:
            while True:
                #data structure dict
                chuck = {'time' : str(time.time()), 'categorylegend' : None, 'changedtitle' : None, 'gamename' : None, 'x' : None, 'y' : None}
                ...
                try:
                    ...
                    try:
                        ...
                        if now_title != old_title and now_title != None:
                            ...
                            chname = now_title
                            chuck['changedtitle'] = [datetime.datetime.now(), now_title]
                            self.change_title.append([datetime.datetime.now().isoformat(), now_title])
                            chuck['gamename'] = gn
                            self.gns.append(gn)
                            chuck['lc'] = self.legendcount
                    except:
